...
netstat -bano | select-string  -SimpleMatch "0:6","0:1433","0:135" `
    -Context 0,1  #,"sql""RPC","msdtc","wininit"
               
#configure Windows firewall
Set-NetFirewallRule -DisplayGroup "Windows Management Instrumentation (WMI)" -Enabled True
Set-NetFirewallRule -DisplayGroup "Distributed Transaction Coordinator" -Enabled True

New-NetFirewallRule -Name "contoso-DTC-DynamicPorts" -DisplayName  "contoso-DTC-DynamicPorts" `
    -LocalPort 60000-60022 -Enabled True -Protocol tcp -Direction Inbound
New-NetFirewallRule -Name "Ocontoso-DTC-DynamicPorts" -DisplayName  "Ocontoso-DTC-DynamicPorts" `
    -LocalPort 60000-60022 -Enabled True -Protocol tcp -Direction Outbound
...
# DTC Security --------------------------------------
#----------------------------------------------------
Get-DtcNetworkSetting  | Format-Table -AutoSize
Set-DtcNetworkSetting -AuthenticationLevel NoAuth -DtcName "Local" `
    -InboundTransactionsEnabled $True -LUTransactionsEnabled $True `
    -OutboundTransactionsEnabled $True -RemoteAdministrationAccessEnabled $False `
    -RemoteClientAccessEnabled $False -XATransactionsEnabled $True
Get-DtcNetworkSetting  | Format-Table -AutoSize
...
test-dtc -LocalComputerName $env:computername -RemoteComputerName "Server1" `
    -verbose -ResourceManagerPort 80 # 17100 #port 0 by default
...
GO
#>
Invoke-Sqlcmd -ServerInstance Server1 -Query `
    "begin distributed tran;select name from [Server1].master.sys.databases;waitfor delay '0:0:30';commit" `
    -QueryTimeout 60
